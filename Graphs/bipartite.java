...
public class bipartite {
    ...
    public static boolean isBipartite(ArrayList<Edge>[] graph){
        int col[] = new int[graph.length]; //-1 no color , 0 yellow , 1 blue
        for(int i=0 ; i<col.length ; i++){ 
            col[i]=-1;
        }
        Queue<Integer> q = new LinkedList<>();

        for(int i= 0 ; i<graph.length ; i++){ //condition for different components
            if(col[i]==-1){ //this loop will check for an entire connected graph
                q.add(i);
                col[i]=0; //as this will happen only once(for each connected graph we hardcode the first element to have color 0(yellows) for a graph)
                while(!q.isEmpty()){
                    int curr = q.remove();
    ...
    public static void main(String[] args) {
        int v = 5; // total 7 vertices (0 to 4)
